"""
Online fusion model (video + IMU) prediction using samples in Temporary_Data.
...
Important:
    ...
    - IMU and video data are assumed to be collected in real time by a
      separate script (e.g. 2_GetData.py) that writes the Sample_* folders.

Behaviour:
    - At each iteration, reads the last N Sample_* folders (default: 2) from
      Temporary_Data and concatenates their frames and IMU data.
    ...
    - Prints actions continuously until interrupted with Ctrl+C.
"""
...
NUM_RECENT_SAMPLES = 2                              # how many latest Sample_* folders to fuse
SLEEP_NO_DATA = 0.5                                 # seconds to wait if no data yet
# ------------------------------------
...
try:
    # ONLINE dataloader (last N samples)
    from Imports.InferenceDataloader_Online import HAR_Inference_DataSet
...
def main() -> None:
    ...
    try:
        while True:
            # Try to build online dataset from the last N samples
            try:
                dataset = HAR_Inference_DataSet(
                    root_dir=str(root_directory),
                    transform=transform,
                    num_samples=NUM_RECENT_SAMPLES,
                )
            except (IndexError, FileNotFoundError, RuntimeError) as e:
                # No data yet or incomplete data – wait and retry
                print(f"No valid Sample_* data yet ({e}). Waiting...")
                time.sleep(SLEEP_NO_DATA)
                continue

            # For logging, take the latest sample name if available
            sample_names = getattr(dataset, "sample_names", None)
            latest_sample_label = sample_names[-1] if sample_names else "LatestSamples"

            try:
                prediction_idx = make_prediction(dataset, model, device)
            except FileNotFoundError:
                print(f'{latest_sample_label}: data vanished while reading – skipping.')
                time.sleep(SLEEP_NO_DATA)
                continue
            ...
            if same_flag:
                ...
                if accept:
                    ...
                    print(
                        f'{latest_sample_label} : {nice_label}  '
                    ...
                    print(
                        f'{latest_sample_label} : {current_action} '
                ...
                print(prediction_save)
                print(f'{latest_sample_label} : {idx_to_action.get(prediction_idx, "unknown")}')

            # Small sleep to avoid hammering the disk
            time.sleep(0.05)
    ...
    print(f'\nTotal raw predictions: {num_of_predictions}')
    for action, idx in action_to_idx.items():
        print(f'{tracking[idx]} raw model outputs for {action}')

    print('\nOnline mode stopped by user. No UDP, no RPi communication.')
