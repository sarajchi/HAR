...
def main() -> None:
    ...
    # Mode is fixed to Offline
    mode = "Offline"
    print("Mode selected: Offline\n")

    print("\033c🟢 Real-time Prediction with Averaging Started...")

    current_vote = "Down"
    motor_state = "Down"  # purely logical now
    state_assurance: list[str] = []
    i = 0  # index for Offline mode
    ...
    try:
        while True:
            ...
            if not folders:
                print("No folders found in Temporary_Data. Waiting...")
                time.sleep(1.0)
                continue

            # Offline only: step through folders sequentially
            recent_folders = folders[i: i + WINDOW_SIZE]
            if not recent_folders:
                print("No more folders for Offline mode. Stopping.")
                break
            ...
            print(f"Processing time this cycle: {elapsed:.3f} s")
