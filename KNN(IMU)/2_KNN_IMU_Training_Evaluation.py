...
"""
KNN classification on IMU features with metrics and plots.
...
def main() -> None:
    ...
    # Load IMU dataset
    # --------------------------------------------------------------
    df = pd.read_csv("All_IMU_Features.csv")
    ...
    # Class order: Down (2) / Grab (0) / Walk (1)
    cls_labels = [2, 0, 1]
    ...
    print(f"{'Macro Avg':<12}{macro['precision']:>10.2f}{macro['recall']:>10.2f}{macro['f1-score']:>10.2f}{total_support:>10}")
    print(f"{'Weighted Avg':<12}{weighted['precision']:>10.2f}{weighted['recall']:>10.2f}{weighted['f1-score']:>10.2f}{total_support:>10}")

    print(f"\nAccuracy: {report['accuracy']:.8f}")
    ...
    fig_cm.tight_layout()
    plt.show()
    fig_cm.savefig("knn_IMU_confusion_matrix.png", dpi=300)
    ...
    ax_k.set_title("KNN Accuracy vs Number of Neighbours (K)")
    ...
    fig_k.tight_layout()
    plt.show()
    fig_k.savefig("knn_accuracy_vs_k.png", dpi=300)

    # --------------------------------------------------------------
    # Recall per Class
    # --------------------------------------------------------------
    recall_values = [report["Down"]["recall"], report["Grab"]["recall"], report["Walk"]["recall"]]
    ...
    plt.show()
    fig_rec.savefig("knn_recall_per_class.png", dpi=300)
    ...
    for cid, name in zip(label_ids, cls_names):
        precision, recall, _ = precision_recall_curve(y_test_bin[:, cid], y_scores[:, cid])
        ap = average_precision_score(y_test_bin[:, cid], y_scores[:, cid])
        ax_pr.plot(recall, precision, lw=2, label=f"{name} (AP={ap:.2f})")

    ax_pr.set_title("Precision–Recall Curve - KNN")
    ...
    fig_pr.tight_layout()
    plt.show()
    fig_pr.savefig("knn_precision_recall_curve.png", dpi=300)

    # --------------------------------------------------------------
    # Save model & scaler
    # --------------------------------------------------------------
    joblib.dump(knn, "knn_imu_model.pkl")
    joblib.dump(scaler, "knn_imu_scaler.pkl")
